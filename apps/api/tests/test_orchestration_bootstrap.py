"""MVP-04 deterministic orchestration bootstrap: RESEARCH -> AUDIENCE ->
STRATEGY -> PLAN. All marked `postgres`.

Preserves every non-negotiable from the MVP-04 implementation gate:
no Content/Assets/Tracking/Measurement/Learning row, no fabricated
...
def test_start_bootstraps_research_through_plan_and_leaves_content_pending(campaign_run_client: dict) -> None:
...
def test_bootstrap_creates_zero_content_briefs_for_its_own_plan(campaign_run_client: dict, db_session) -> None:
    """Scoped to this test's own ContentPlan — a raw table-wide COUNT(*)
    would pick up rows committed by unrelated tests sharing the same
    Postgres database across the full suite run."""
    initialize_run(campaign_run_client)
    start_run(campaign_run_client)
...
def test_bootstrap_code_never_references_content_tracking_measurement_or_learning_write_methods() -> None:
    ...
    contain no reference to any Content-approval-adjacent, Tracking,
    Measurement, or Learning write command."""
    import inspect
    ...
    forbidden = (
        "record_brief", "record_piece", "mark_in_production", "mark_produced",
        "mark_ready_for_review", "record_authorized_approval_decision",
...
def test_bootstrap_outputs_are_isolated_per_workspace(auth_client) -> None:
    ...
    assert {s["stage"]: s["status"] for s in stages_a}["PLAN"] == "COMPLETED"
    stages_b = fixtures_b["client"].get(run_path(fixtures_b, "/stages")).json()["items"]
    assert {s["stage"]: s["status"] for s in stages_b}["PLAN"] == "COMPLETED"
