"""Data access for Audit Event — append-only by construction: this
module deliberately exposes no update/delete method at all (BACKEND-06
§21). No repository here calls ``session.commit()`` — see
``app/persistence/session.py``; the caller (an orchestration service
operation) commits the event in the same transaction as the state
change it records, so the two can never diverge (BACKEND-06 §28).
"""

from __future__ import annotations

import uuid

from sqlalchemy import func, select
from sqlalchemy.orm import Session

from app.audit.models import ActorType, AuditEvent
from app.core.ids import generate_public_id


class AuditEventRepository:
    def __init__(self, session: Session) -> None:
        self.session = session

    def record(
        self,
        *,
        workspace_id: uuid.UUID | None,
        event_type: str,
        actor_type: ActorType,
        campaign_id: uuid.UUID | None = None,
        campaign_run_id: uuid.UUID | None = None,
        stage_execution_id: uuid.UUID | None = None,
        decision_request_id: uuid.UUID | None = None,
        research_report_id: uuid.UUID | None = None,
        audience_profile_id: uuid.UUID | None = None,
        strategy_id: uuid.UUID | None = None,
        hypothesis_id: uuid.UUID | None = None,
        experiment_id: uuid.UUID | None = None,
        content_plan_id: uuid.UUID | None = None,
        plan_item_id: uuid.UUID | None = None,
        content_brief_id: uuid.UUID | None = None,
        content_piece_id: uuid.UUID | None = None,
        content_version_id: uuid.UUID | None = None,
        content_approval_id: uuid.UUID | None = None,
        distribution_id: uuid.UUID | None = None,
        metric_entry_id: uuid.UUID | None = None,
        performance_observation_id: uuid.UUID | None = None,
        performance_signal_id: uuid.UUID | None = None,
        analysis_result_id: uuid.UUID | None = None,
        creative_brief_id: uuid.UUID | None = None,
        asset_id: uuid.UUID | None = None,
        asset_version_id: uuid.UUID | None = None,
        learning_candidate_id: uuid.UUID | None = None,
        strategic_recommendation_candidate_id: uuid.UUID | None = None,
        strategic_implication_id: uuid.UUID | None = None,
        tracking_plan_id: uuid.UUID | None = None,
        tracking_requirement_id: uuid.UUID | None = None,
        measurement_analysis_run_id: uuid.UUID | None = None,
        distribution_metric_evidence_id: uuid.UUID | None = None,
        commercial_objective_id: uuid.UUID | None = None,
        offer_id: uuid.UUID | None = None,
        commercial_outcome_id: uuid.UUID | None = None,
        experiment_definition_version_id: uuid.UUID | None = None,
        experiment_variant_id: uuid.UUID | None = None,
        measurement_contract_id: uuid.UUID | None = None,
        execution_authorization_id: uuid.UUID | None = None,
        execution_start_attestation_id: uuid.UUID | None = None,
        experiment_evidence_claim_id: uuid.UUID | None = None,
        experiment_measurement_run_id: uuid.UUID | None = None,
        strategic_decision_id: uuid.UUID | None = None,
        strategic_approval_id: uuid.UUID | None = None,
        strategy_revision_id: uuid.UUID | None = None,
        previous_state: str | None = None,
        new_state: str | None = None,
        actor_user_id: uuid.UUID | None = None,
        request_id: str | None = None,
    ) -> AuditEvent:
        event = AuditEvent(
            public_id=generate_public_id("AUDT"),
            workspace_id=workspace_id,
            campaign_id=campaign_id,
            campaign_run_id=campaign_run_id,
            stage_execution_id=stage_execution_id,
            decision_request_id=decision_request_id,
            research_report_id=research_report_id,
            audience_profile_id=audience_profile_id,
            strategy_id=strategy_id,
            hypothesis_id=hypothesis_id,
            experiment_id=experiment_id,
            content_plan_id=content_plan_id,
            plan_item_id=plan_item_id,
            content_brief_id=content_brief_id,
            content_piece_id=content_piece_id,
            content_version_id=content_version_id,
            content_approval_id=content_approval_id,
            distribution_id=distribution_id,
            metric_entry_id=metric_entry_id,
            performance_observation_id=performance_observation_id,
            performance_signal_id=performance_signal_id,
            analysis_result_id=analysis_result_id,
            creative_brief_id=creative_brief_id,
            asset_id=asset_id,
            asset_version_id=asset_version_id,
            learning_candidate_id=learning_candidate_id,
            strategic_recommendation_candidate_id=strategic_recommendation_candidate_id,
            strategic_implication_id=strategic_implication_id,
            tracking_plan_id=tracking_plan_id,
            tracking_requirement_id=tracking_requirement_id,
            measurement_analysis_run_id=measurement_analysis_run_id,
            distribution_metric_evidence_id=distribution_metric_evidence_id,
            commercial_objective_id=commercial_objective_id,
            offer_id=offer_id,
            commercial_outcome_id=commercial_outcome_id,
            experiment_definition_version_id=experiment_definition_version_id,
            experiment_variant_id=experiment_variant_id,
            measurement_contract_id=measurement_contract_id,
            execution_authorization_id=execution_authorization_id,
            execution_start_attestation_id=execution_start_attestation_id,
            experiment_evidence_claim_id=experiment_evidence_claim_id,
            experiment_measurement_run_id=experiment_measurement_run_id,
            strategic_decision_id=strategic_decision_id,
            strategic_approval_id=strategic_approval_id,
            strategy_revision_id=strategy_revision_id,
            event_type=event_type,
            previous_state=previous_state,
            new_state=new_state,
            actor_type=actor_type,
            actor_user_id=actor_user_id,
            request_id=request_id,
        )
        self.session.add(event)
        self.session.flush()
        return event

    def list_for_run(self, *, campaign_run_id: uuid.UUID, limit: int, offset: int) -> tuple[list[AuditEvent], int]:
        total = self.session.execute(
            select(func.count()).select_from(AuditEvent).where(AuditEvent.campaign_run_id == campaign_run_id)
        ).scalar_one()

        items = (
            self.session.execute(
                select(AuditEvent)
                .where(AuditEvent.campaign_run_id == campaign_run_id)
                .order_by(AuditEvent.created_at.asc(), AuditEvent.id.asc())
                .limit(limit)
                .offset(offset)
            )
            .scalars()
            .all()
        )
        return list(items), total
